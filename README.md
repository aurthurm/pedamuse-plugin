...
You can simply copy the files out of this repo and rename everything as you need it, but to make things easier I have included a [shell script](https://github.com/hlashbrooke/pedamuse/blob/master/build-plugin.sh) in this repo that will automatically copy the files to a new folder, remove all traces of the existing git repo, rename everything in the files according to your new plugin name, and initialise a new git repo in the folder if you choose to do so.
...
Using the [post type API](https://github.com/hlashbrooke/pedamuse/blob/master/includes/lib/class-pedamuse-post-type.php) and the wrapper function from the main plugin class you can easily register new post types with one line of code. For exapmle if you wanted to register a `listing` post type then you could do it like this:

`Pedamuse()->register_post_type( 'listing', __( 'Listings', 'pedamuse' ), __( 'Listing', 'pedamuse' ) );`

*Note that the `Pedamuse()` function name and the `pedamuse` text domain will each be unique to your plugin after you have used the cloning script.*
...
Using the [taxonomy API](https://github.com/hlashbrooke/pedamuse/blob/master/includes/lib/class-pedamuse-taxonomy.php) and the wrapper function from the main plugin class you can easily register new taxonomies with one line of code. For example if you wanted to register a `location` taxonomy that applies to the `listing` post type then you could do it like this:

`Pedamuse()->register_taxonomy( 'location', __( 'Locations', 'pedamuse' ), __( 'Location', 'pedamuse' ), 'listing' );`
...
See the [changelog](https://github.com/hlashbrooke/pedamuse/blob/master/changelog.txt) for a complete list of changes as the template develops.

## I've got an idea/fix for the template

If you would like to contribute to this template then please fork it and send a pull request. I'll merge the request if it fits into the goals for the template and credit you in the [changelog](https://github.com/hlashbrooke/pedamuse/blob/master/changelog.txt).
